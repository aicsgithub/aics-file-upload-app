...
class SelectStorageIntent extends React.Component<
  Props,
  SelectStorageIntentState
> {
  ...
  public render() {
    ...
    return (
      <FormPage
        formPrompt={`Where should ${
          moreThanOneFile ? "these files" : "this file"
        } be stored?`}
  ...
  private renderSimpleForm = () => {
    const { fileToArchive, fileToStoreOnIsilon } = this.props;
    const allFilesShouldBeInIsilon = every(
      fileToStoreOnIsilon,
      (store: boolean) => Boolean(store)
    );
    const allFilesShouldBeInArchive = every(fileToArchive, (store: boolean) =>
      Boolean(store)
    );
    return (
      <>
        ...
        >
          Archive{" "}
          <Text type="secondary">
            (File will be backed up but not downloadable)
          </Text>
        </Checkbox>
        <Checkbox
        ...
        >
          Isilon{" "}
          <Text type="secondary">
            (Necessary if this file will be downloaded again)
          </Text>
        </Checkbox>
      </>
    );
  ...
  private renderGridForm = () => {
    const { fileToArchive, fileToStoreOnIsilon, files } = this.props;
  ...
  private setCustomizeByFile = (checked: boolean) =>
    this.setState({ customizeByFile: checked });

  private selectAllIsilon = (e: CheckboxChangeEvent) => {
    this.props.updateFilesToStoreOnIsilon(
      this.props.files.reduce(
        (accum: FilepathToBoolean, file: string) => ({
          ...accum,
