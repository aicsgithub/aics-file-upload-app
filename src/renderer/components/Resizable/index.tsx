// TODO: This component should be refactored to remove violations of this rule
...
class Resizable extends React.Component<ResizableProps, ResizableState> {
  ...
  public componentDidMount() {
    if (!this.props.width && (this.props.left || this.props.right)) {
      throw Error(
        "Width required if component is supposed to resize horizontally"
      );
    }
    if (!this.props.height && (this.props.top || this.props.bottom)) {
      throw Error(
        "Height required if component is supposed to resize vertically"
      );
    }
  ...
  public render() {
    ...
    return (
      <div
        ...
        ref={divRef}
        style={{ cursor, height, ...padding, width }}
      >
  ...
  private onMouseDown = (event: MouseEvent) => {
    ...
    const onLeftBorder =
      Math.abs(clientLeft - clientX) <= this.borderSizeInPixels;
    const onRightBorder =
      Math.abs(clientWidth + clientLeft - clientX) <= this.borderSizeInPixels;
    const onTopBorder =
      Math.abs(clientTop - clientY) <= this.borderSizeInPixels;
    const onBottomBorder =
      Math.abs(clientHeight + clientTop - clientY) <= this.borderSizeInPixels;

    if (
      (this.props.left && onLeftBorder) ||
      (this.props.right && onRightBorder)
    ) {
      this.setState({ userResizeOriginX: event.clientX });
    }
    if (
      (this.props.top && onTopBorder) ||
      (this.props.bottom && onBottomBorder)
    ) {
