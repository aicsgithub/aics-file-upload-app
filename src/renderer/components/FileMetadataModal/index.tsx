...
const FileMetadataModal: React.FunctionComponent<FileMetadataProps> = ({
  ...
  const fileMetadataCategories: { [key: string]: ListItem[] } = {
    annotations: [],
    extra: [],
    info: [],
  };
  ...
  return (
    <Modal
      ...
      title={
        <h3>
          File Details For <strong>{fileMetadata.filename}</strong>
        </h3>
      }
    >
      <List
      ...
      {isArchive && !isPublic && !isLocal && (
        <Alert
          type="warning"
          message="File is only available for download from the archive"
        />
      )}
      {!isArchive && !isPublic && !isLocal && (
        <Alert
          type="error"
          message="File doesn't appear to be available for download from anywhere"
        />
      )}
      {isPublic && (
