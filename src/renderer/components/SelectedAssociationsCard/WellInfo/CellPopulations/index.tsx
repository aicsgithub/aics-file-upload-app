  ...
  return (
    <div className={className}>
      {cellPopulations.map((entry: CellPopulation, i) => {
        const { sourcePlateWell, sourceVial, wellCellPopulation } = entry;
        ...
        if (wellCellPopulation) {
          ...
          if (
            sourcePlateWell &&
            sourcePlateWell.plateBarcode &&
            sourcePlateWell.wellLabel
          ) {
            sourceText = (
              <React.Fragment>
                <KeyValueDisplay
                  keyName="Source Plate"
                  value={sourcePlateWell.plateBarcode}
                />
                <KeyValueDisplay
                  keyName="Well"
                  value={sourcePlateWell.wellLabel}
                />
              </React.Fragment>
            );
          } else if (sourceVial) {
          ...
          populationText = (
            <React.Fragment>
              ...
              {wellCellPopulation.cellLineName && (
                <KeyValueDisplay
                  keyName="Cell Line"
                  value={wellCellPopulation.cellLineName}
                />
              )}
              <KeyValueDisplay
                keyName="Clone"
                value={wellCellPopulation.clone || NULL_TEXT}
              />
              <KeyValueDisplay
                keyName="Passage"
                value={wellCellPopulation.passage || NULL_TEXT}
              />
            </React.Fragment>
          );
        }
        return (
          <React.Fragment key={i}>
            ...
            <div className={styles.group}>
              {populationText}
              <KeyValueDisplay
                keyName="Seeding Density"
                value={entry.seedingDensity || NULL_TEXT}
              />
            </div>
          </React.Fragment>
        );
