...
import { AlertType, AppEvent } from "../../../state/types";

describe("<StatusBar/>", () => {
  const message = "Something happened";
  let event: AppEvent;
  const getStatusText = () =>
    shallow(<StatusBar event={event} limsUrl="url" />)
      .find(".status")
      .text();
  ...
  it("returns (3 minutes ago) if date is around 3 minutes ago", () => {
    event = {
      ...event,
      date: new Date(
        Date.now() - 3 * SECONDS_IN_A_MINUTE * MILLISECONDS_PER_SECOND
      ),
    };
    const result = getStatusText();
  ...
  it("returns time event occurred at if more than an hour ago", () => {
    const date = new Date(
      Date.now() - 3 * SECONDS_IN_AN_HOUR * MILLISECONDS_PER_SECOND
    );
    event = {
      ...event,
      date,
