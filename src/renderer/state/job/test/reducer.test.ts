...
import { receiveJobs, selectJobFilter, startJobPoll, stopJobPoll, updateIncompleteJobIds } from "../actions";
...
describe("job reducer", () => {
    describe("receiveJobs", () => {
        it("sets addMetadataJobs, copyJobs, incompleteJobIds, uploadJobs, and inProgressJobs", () => {
            ...
            const result = reducer(
                initialState,
                receiveJobs(uploadJobs, copyJobs, addMetadataJobs, incompleteJobIds, inProgressJobs)
            );
    ...
    describe("retryUpload", () => {
        it("sets incompleteJobIds", () => {
            const result = reducer(initialState, retryUpload({...mockFailedUploadJob, key: "key"}));
            expect(result.incompleteJobIds).to.include(mockFailedUploadJob.jobId);
        });
    });
    describe("retryUploadSucceeded", () => {
        it("sets incompleteJobIds", () => {
            const result = reducer(initialState, retryUploadSucceeded({...mockFailedUploadJob, key: "key"}));
            expect(result.incompleteJobIds).to.not.include(mockFailedUploadJob.jobId);
        });
    });
    describe("retryUploadFailed", () => {
        it("sets incompleteJobIds", () => {
            const result = reducer(initialState, retryUploadFailed({...mockFailedUploadJob, key: "key"}, "error"));
            expect(result.incompleteJobIds).to.not.include(mockFailedUploadJob.jobId);
        });
    });
});
